            ...
            VStack{
                HStack{
                    Text("Order for " + (name == "" ? "You" : name ) )
                        .font((vSizeClass == .compact) || (hSizeClass == .compact) ? .caption : .largeTitle)
                    ...
                    Button{
                        ...
                        Spacer()
                        Text((item?.price ?? 0) * Double(quantity)
                             ,format:.currency(code: "USD"))
                        .font(vSizeClass == .compact ? .caption : .title).bold()
                ...
                HStack(alignment:.top){
                    if vSizeClass != .compact {
                        ...
                        .font(vSizeClass == .compact ? .caption : .title)
                        .padding()
                        .padding(.trailing,20)
                    } else {
                        HStack(alignment:.top){
                            Text(item?.name ?? "Huli Pizza")
                                .font(vSizeClass == .compact ? .caption : .largeTitle)
